...
for sub = 1:length(allsubs)%:length(allsubs)
    ...
    resultsdir = [subdir 'mvpa/roi_betas_WMplus_maintaskdata_GLM2_control/'];
    betasdir1 = 'GLM1.feat'; % .feat directory to pull COPEs from
    betasdir2 = 'GLM2_onlypost_control.feat';
...
for sub=1:length(allsubs)
    for iROI = 1:nROIs
        for iImage = 0:9
            combine=0;
            ROIname = rois{iROI};
            ThisResultsDir = fullfile(allsubs{sub}, 'mvpa', 'roi_betas_WMplus_maintaskdata_GLM2_control', ['image' num2str(iImage)], ROIname);
            n_perms = 100;
...
for sub=1:length(allsubs)
    for iImage = 0:9 %Run decoding for each ROI and each image separately, then average results across images within each ROI\
        for iROI = 1:nROIs
           balanced_acc = load([allsubs{sub}, '/', 'mvpa/roi_betas_WMplus_maintaskdata_GLM2_control/image', num2str(iImage), '/', rois{iROI}, '/res_balanced_accuracy.mat']);
...
for sub = 1:3
    for iROI = 1:nROIs
        ...
        for i = 1:nReps
            ...
            for iImage = 0:9    
                accuracy = load([allsubs{sub} '/mvpa/roi_betas_WMplus_maintaskdata_GLM2_control/image' num2str(iImage) '/' ROIname '/perm/perm' num2str(randselection(iImage+1),'%04.f') '_balanced_accuracy.mat']);
...
save('results_mvpa/balacc_beta_main_GLM2control.mat','results');
